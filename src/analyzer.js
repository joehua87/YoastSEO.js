import PreProcessor from './preprecessor';
import StringHelper from './string-helper';
import AnalyzeScorer from './analyze-scorer';
import { analyzerConfig } from './config/config';
...
export default class Analyzer {
  ...
  formatKeyword() {
    if (typeof this.config.keyword !== "undefined" && this.config.keyword !== "") {
      ...
      const keyword = this.stringHelper.sanitizeKeyword(this.config.keyword);
  ...
  initDependencies() {
    ...
    // init preprocessor
    this.preProcessor = new PreProcessor(this.config.text);
  ...
  initQueue() {
    ...
    if (this.config.locale.indexOf("en_") !== 0 && ( fleschReadingIndex = this.queue.indexOf("fleschReading") )) {
 ...
 * 	}
   ...
   * @returns count
   */
  paragraphChecker(textString, regexp) {
