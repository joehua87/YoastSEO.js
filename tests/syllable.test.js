...
import { preprocessorConfig } from '../src/config/config';
...
describe("Test for the syllablecount", () => {
  const preproc2 = new PreProcessor(preprocArgs2.testString);
  it("returns syllable count of first string", () => {
    expect(preproc2.__store.syllablecount).to.equal(62);
  });
  const preproc3 = new PreProcessor(preprocArgs2.testString2);
  it("returns syllable count of 2nd string", () => {
    expect(preproc3.__store.syllablecount).to.equal(65);
  });
  const preproc4 = new PreProcessor(preprocArgs2.testString3);
  it("returns syllable count of 3rd string", () => {
    expect(preproc4.__store.syllablecount).to.equal(57);
  });
  const preproc5 = new PreProcessor(preprocArgs2.testString4);
  ...
  it("returns syllable count of 4th string by using exclusionwords", () => {
    expect(preproc5.__store.syllablecount).to.equal(14);
  });
});
