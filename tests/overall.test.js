...
describe("a test running multiple textstrings", () => {
  /* these tests are currently not used, because javascript doesn't work with a wordboundary on specialchars.
   ...
   it("checks for keywords in hebrew - should return 8 matches - keywordDensity = 7.1", () => {
   const keywordAnalyzer = buildAnalyzer( textStrings[0] );
   const result = keywordAnalyzer.keywordDensity();
   expect(result[0].result).to.equal("7.1");
   });

   it("checks for keywords in Cyrillic - should return 6 matches - keywordDensity = 4.1", () => {
   const keywordAnalyzer = buildAnalyzer( textStrings[1] );
   const result = keywordAnalyzer.keywordDensity();
   expect(result[0].result).to.equal("4.1");
   });
   */

  it("checks for keywords in German - should return 3 matches = keywordDensity = 1.9", () => {
    const keywordAnalyzer = buildAnalyzer(textStrings[2]);
    const result = keywordAnalyzer.keywordDensity();
    expect(result[0].result).to.equal("1.9");
  });

  it("checks for keywords with hyphens - should return 2 matches = keywordDensity = 1.1", () => {
    const keywordAnalyzer = buildAnalyzer(textStrings[3]);
    const result = keywordAnalyzer.keywordDensity();
    expect(result[0].result).to.equal("1.1");
  });
  /* this tests are currently not used, because javascript doesn't work with a wordboundary on specialchars.
   // see issue https://github.com/Yoast/YoastSEO.js/issues/104
   it("checks for keywords with special chars - should return 4 matches = keywordDensity = 2.2", () => {
   const keywordAnalyzer = buildAnalyzer( textStrings[4] );
   const result = keywordAnalyzer.keywordDensity();
   expect(result[0].result).to.equal("2.2");
   });
   */
  it("checks for keywords with tables - keyword should be found in first paragraph", () => {
    const keywordAnalyzer = buildAnalyzer(textStrings[5]);
    const result = keywordAnalyzer.firstParagraph();
    expect(result[0].result).to.equal(1);
  });

  it("checks for keywords with attr on <p> - keyword should be found in first paragraph", () => {
    const keywordAnalyzer = buildAnalyzer(textStrings[6]);
    const result = keywordAnalyzer.firstParagraph();
    expect(result[0].result).to.equal(1);
  });
});
